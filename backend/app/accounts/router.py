...
async def login(request: Request, data: LoginSchema, response: Response):
    ...
    access = create_token(data={"sub": str(user.id)}, expires=timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES))
    refresh = create_token(data={"sub": str(user.id), "type": "refresh"}, expires=timedelta(days=settings.REFRESH_TOKEN_EXPIRE_DAYS))
...
async def me(user=Depends(get_user)):
    if user.name and user.email is not None:
        return {
            "name": user.name,
            "email": user.email,
        }